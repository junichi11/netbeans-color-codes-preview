...
public class ColorsUtilsTest {
    ...
    public void testPercentValueRegex() {
        Pattern pattern = Pattern.compile(HexCssColorType.PERCENT_VALUE_FORMAT);
        for (int i = 0; i < 100; i++) {
    ...
    public void testAlphaValueRegex() {
        Pattern pattern = Pattern.compile(String.format("(%s)", HexCssColorType.ALPHA_VALUE_FORMAT));
        // 0.1-0.9
        for (int i = 1; i < 10; i++) {
    ...
    public void testHueValueRegex() {
        Pattern pattern = Pattern.compile(HexCssColorType.HUE_VALUE_FORMAT);
        for (int i = 0; i <= 360; i++) {
            Matcher matcher = pattern.matcher(String.valueOf(i));
            Assert.assertTrue(matcher.matches());
        }
    ...
    @Test
    public void testGetJavaStandardColors() {
        List<ColorValue> result = ColorsUtils.getJavaStandardColors("Color color = Color.black;", 1);
        assertEquals(1, result.size());
        ...
        assertEquals(0, result.size());
        result = ColorsUtils.getJavaStandardColors("Color color = new Color(255, 255, 255, 255);", 1);
        assertEquals(0, result.size());
    }
